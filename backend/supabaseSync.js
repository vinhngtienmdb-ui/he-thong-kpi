 ...
 */
function triggerBackgroundSupabaseSync(delayMs = 4000) {
  ...
  syncTimer = setTimeout(async () => {
    ...
    try {
      ...
      console.log('[Auto-Sync Supabase] Dữ liệu vừa thay đổi đã tự động sao lưu lên Supabase Cloud thành công.');
    } catch (err) {
      ...
      if (pendingSync) {
        pendingSync = false;
        triggerBackgroundSupabaseSync(1000);
      }
    }
  }, delayMs);
}
